...
$$
Now we are interested in how the earth absorb radiative energy. Only half of the planet is illuminated hence:
$$
\mathrm{Absorbed\; Radiative \;Energy} = \iint_D S_0|\cos(\phi)|\sin(\phi)r^2 d\phi d\theta 
$$
where $D=\{\theta, \phi : \pi/2\leq \phi\leq \pi/2, 0 \leq \theta \leq \pi\}$. $\phi$ is the latitude with $0$ being north pole and $\pi$ be south pole. $\theta$ is longitude ranging from $0$ to $2\pi$. Note $S_0|\cos(\phi)|$ has adjusted radiation per projection. 

:::{tip}**ToolBox** Double Integral in Spherecial Coordinate 
$r^2 \sin\phi d\phi d\theta $ is the surface element, since a surface element has 
