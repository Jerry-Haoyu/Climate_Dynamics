...
The proof is out of the scope of this course note, however, one can consider the special case when $A$ is diagonalizable : $A=P^{-1}DP$, then $(I-Adt)^{-1}= (I-PDdtP^{-1})^{-1}=$P^{-1}(I-Ddt)^{-1}P$$ where $(I-Ddt)^{-1} = \mathrm{diag}(1)$
