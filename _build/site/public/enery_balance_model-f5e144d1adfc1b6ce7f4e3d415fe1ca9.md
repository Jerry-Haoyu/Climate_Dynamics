...
which has a local truncation error $\mathcal O(\Delta t^2)$. Let's denote the forcing term as $F(\phi, T)\equiv \frac{S_0|\cos \phi| + \sigma T^4}{C}$

Writing out each term we get, for $\phi_i$:
$$
T_{t}(t,\phi_i)= 
$$
