...
The name semidiscrete is quite self-explanotry in the sense that we have two conitnous varaibles $T,\phi$ and we would discretize one. In this case, we use the so called **line method** where we discretize the spatial dimension. The result is 