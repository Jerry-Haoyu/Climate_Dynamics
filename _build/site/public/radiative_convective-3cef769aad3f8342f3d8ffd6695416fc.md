...
$$
where $D=\{\theta, \phi : \pi/2\leq \phi\leq \pi/2, 0 \leq \theta \leq \pi\}$. $\phi$ is the latitude with $0$ being north pole and $\pi$ be south pole. $\theta$ is longitude ranging from $0$ Note $S_0|\cos(\phi)|$ has adjusted radiation per projection. 
