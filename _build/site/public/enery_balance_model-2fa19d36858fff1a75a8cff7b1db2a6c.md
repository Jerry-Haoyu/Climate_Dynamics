...
We first need to write $ASR$ and $OLR$ explicitly as a function of $(T, \phi)$.For absorbed incoming short wave radiation, recall the flux density is $S_0$. Now consider a thin strip parallel to the equator that correspond to a $\delta \phi$. The total flux to the strip is:
$$
\int_{0}^{\pi}d\theta \int_{\phi}^{\phi + \delta \phi}d\phi r^2\sin(\phi)|\cos(\phi)|
$$
