...
$$
T_{t}(t,\phi_i) &= F(\phi_i, T(t, \phi_i)) +  T(\phi_{i+1} ,t) \left\{ \frac{D}{\Delta t^2} + \frac{D\tan \phi_i}{2C\Delta t} \right\} \\
&+ T(\phi_i, t)\left\{ -\frac{2D}{\Delta t^2}\right\} \\
&+ T(\phi_{i-1},t)\left\{ \frac{D}{\Delta t^2} + \frac{D\tan \phi_i}{2C\Delta t} \right\}
$$


In matrix form, let's denote $$\mathbf{T} \equiv \begin{pmatrix} T(\phi_0, t) \\  T(\phi_1, t)  \\ \vdots \\ T(\phi_{N-1}, t)  \end{pmatrix}$$ 
...
\vdots \\
S_0|\cos(\phi_{N-1})|  \\
\end{pmatrix}  + \underbrace{\sigma \cdot  \mathbf{T}^4 }_{\text{elementwise}}
$$

we aim to order the $n$ equations into a linear system in the form $y' = Ay +b$:

$$
\frac{d\mathbf{T}}{dt} = 
\frac{D}{\Delta t^2}
\begin{pmatrix}
    -2 & 1 - \frac{\tan(\phi_0)\Delta t}{2C} &0 &  \cdots & 0 \\
    1 + \frac{\tan(\phi_1) \Delta t}{2C} & -2 & 1 - \frac{\tan(\phi_1) \Delta t}{2C} &\ddots & 0 \\
    0 &  1 + \frac{\tan(\phi_2) \Delta t}{2C} & -2 & \ddots & 0 \\
    \vdots & \ddots & \ddots & \ddots &  1 - \frac{\tan(\phi_1) \Delta t}{2C} \\
    0 & \cdots & 0 &  1 + \frac{\tan(\phi_1) \Delta t}{2C} & -2 
\end{pmatrix} \mathbf{T} \\ 
+ \mathbf{F}
$$

Now $\mathbf{F}$ contains $\mathbf{T}$ and is none-linear. We resolve this by using linear approximation $\sigma \mathbf{T}^4 = A\mathbf{T}+B$. 
We can taylor expland at some $T_0$.

$$ \sigma  T^4 = \approx  (4\sigma T_0^3) T - 3\sigma T_0$$
...
\mathbf{b}= 
\begin{pmatrix}
S_0|\cos(\phi_0)| - 3\sigma T_0  \\
S_0|\cos(\phi_1)| - 3\sigma T_0  \\
\vdots \\
S_0|\cos(\phi_{N-1})|  - 3\sigma T_0 \\
\end{pmatrix}
$$

We now attain the system of ODEs from the original PDE:

:::{note} Model After Semidiscretization 
$$
\frac{d\mathbf{T}}{dt} = 
\frac{D}{\Delta t^2}
\begin{pmatrix}
    -2 + 4\sigma T_0^3  & 1 - \frac{\tan(\phi_0)\Delta t}{2C} &0 &  \cdots & 0 \\
    1 + \frac{\tan(\phi_1) \Delta t}{2C} & -2 + 4\sigma T_0^3  & 1 - \frac{\tan(\phi_1) \Delta t}{2C} &\ddots & 0 \\
    0 &  1 + \frac{\tan(\phi_2) \Delta t}{2C} & -2 + 4\sigma T_0^3  & \ddots & 0 \\
    \vdots & \ddots & \ddots & \ddots &  1 - \frac{\tan(\phi_1) \Delta t}{2C} \\
    0 & \cdots & 0 &  1 + \frac{\tan(\phi_1) \Delta t}{2C} & -2 + 4\sigma T_0^3  
\end{pmatrix} \mathbf{T} \\ 
+ \mathbf{b}
$$
:::
