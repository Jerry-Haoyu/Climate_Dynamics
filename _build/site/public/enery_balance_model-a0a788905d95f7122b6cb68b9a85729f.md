...
[^stiff]: Stiffness means the process we are studying(the 'diffusion' of heat has a small time-scale comparing to the interval which we are simulating), mathematically, this means the eigenvalues of the state-transition