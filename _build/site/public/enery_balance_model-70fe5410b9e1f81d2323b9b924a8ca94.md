...
&=\lim_{\delta \phi \to 0} \frac{\pi \int_{\phi_0}^{\phi_0+\delta \phi} d\phi r^2\sin(\phi)|\cos\phi|}{\pi \int_{\phi_0}^{\phi_0+\delta \phi} d\phi  r^2 \sin(\phi)}
\end{align*}
A standard result from mathematical analysis:
$$
\lim_{\delta a\to 0}\frac{\int_a^{a+\delta a}f}{\int_a^{a+\delta a}g} = \frac{f}{g}, \forall 
$$
which gets us:
\begin{align*}
