...
If you are new to programming, the `class` here just means we are using a abstract object to encapsulate the tiny numerical engine we are using here to solve a problem. The object would collect all necessary compoents like remembering physical constants, storing results etc. It's just a good habbit since by doing so we essentially exposed clean interfaces to users(who want to solve a equation). For instance, to run the above code, we can use the cell below:
