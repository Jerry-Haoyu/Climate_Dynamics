...
:::{note} Summary
This note aims to develop a simple model called the **energy balance model** that depicts the tempearture distribution across latitude due to differential heating.  In particular, we are considering an static aquaplanet with no fluid dynamics. We first discretize the space across latitude to obtain "rings" 
:::
...
when we want to run the model with different configurations, we can just modify the dict in the cell above instead of jumping between cells all around the notebook. 

That being said, we are coding up the solver for educational purposes(that's also why AI can replace software engineering but can't replace typing up code by hand, since coding itself is a very rewarding learning process), therefore feel free to do whatever you want. 
